...
interface AbsolutePadProp extends Prop {
  index: number;
}

const AbsolutePad = styled.div<AbsolutePadProp>`
  position: absolute;
  left: 0;
  right: 0;
...
const Row = ({ index, rowHeight }) => {
  const item = index;
  return <ListItem height={rowHeight}>item-{item}</ListItem>;
};
...
const VList = () => {
  ...
  const renderDisplayContent = () => {
    ...
    return (
      <AbsolutePad index={bufferStartIndex} height={rowHeight}>
        {content}
      </AbsolutePad>
    );
  };
