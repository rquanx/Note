...
基本结构
语义化
不用每个元素都分配类名：  只有当类名出现重复，不便于区分的时候才继续细化
命名：根据它是什么命名，而不是外观定义
	外观定义：可考虑用于一些通用的地方上，是固定的，公共类
Div:块元素划分，只有没有现有元素能划分区域时才 使用div划分,省去不必要的结构
Span:用于划分行内元素
...
高级选择器
子选择器
a>li   只选中直接子元素（对孙元素无效）

+ 选择相邻兄弟元素

属性选择器：
Span[title] 选中具有特定属性的元素
[id="xxx"] 选中特定属性值为xxx的元素（对于多值的属性，会有空格隔开，以包含的方式查找）
...
盒模型
IE盒模型只会出现在IE5.5及其更早的版本中。只要为文档设置一个DOCTYPE

a标签可以作外部引用和内部引用
	内部引用，页面内位置跳转？

任何元素都能设置background为图片，从而实习一些功能
	Li  增加padding和背景图，去除圆点，修改图片位置即可用图片替换圆点
